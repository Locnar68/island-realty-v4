        ...
        try:
            ...
            for match in results.get('matched', []):
                if match.get('db_id') and match.get('reo_status'):
                    ...
                    # Update price from spreadsheet
                    if match.get('act_price') and match['act_price'] > 0:
                        update_parts.append("current_list_price = %s")
                        update_vals.append(match['act_price'])
                    # Update agent_access and hold_harmless from spreadsheet
                    if match.get('agent_access'):
                        update_parts.append("agent_access = %s")
                        update_vals.append(match['agent_access'])
                        update_parts.append("hold_harmless_required = %s")
                        update_vals.append(match.get('hold_harmless', False))
                    ...
                    # Auto-deactivate if Sold or Closed
            ...
            for new_prop in results.get('in_act_not_db', []):
                ...
                try:
                    ...
                    agent_access_val = new_prop.get('agent_access')
                    occupancy_val = new_prop.get('occupancy')
                    hh_val = new_prop.get('hold_harmless', False)
                    cur.execute("""
                        INSERT INTO properties 
                        (address, city, current_status, reo_status, current_list_price,
                         financing_type, property_type, agent_access, occupancy_status,
                         hold_harmless_required, data_source, is_active, created_at, updated_at)
